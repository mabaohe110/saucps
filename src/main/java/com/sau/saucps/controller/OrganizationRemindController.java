...
@RequestMapping("/organization-remind")
public class OrganizationRemindController {

@Autowired
private IOrganizationRemindService organizationRemindService;
...
Page<OrganizationRemind> page = new Page<>(pageNum, pageSize);
    ...
    */
    ...
    public R info(@PathVariable("id") Integer id) {
        OrganizationRemind organizationRemind = organizationRemindService.getById(id);
    if (organizationRemind == null) {
    throw RRException.notFound(Constant.ERROR_MSG_ID_NOT_EXIST);
    }
    return R.ok().put("organizationRemind", organizationRemind);
    }
    ...
    */
    @PostMapping("/save")
    public R save(@Valid @RequestBody OrganizationRemind organizationRemind, HttpServletResponse response) {
    if (organizationRemind.getId() != null) {
    throw RRException.badRequest(Constant.ERROR_MSG_ID_NOT_NEED);
    }
    if (organizationRemindService.save(organizationRemind)) {
    return R.created(response).put("organizationRemind", organizationRemind);
    }
    ...
    */
    ...
    public R delete(@PathVariable Integer id, HttpServletResponse response) {
        OrganizationRemind organizationRemind = organizationRemindService.getById(id);
        File file = new File("E://"+organizationRemind.getDataName());
    if (organizationRemindService.removeById(id) && file.delete()) {
    ...
    */
    ...
    if (organizationRemindService.removeByIds(Arrays.asList(ids))) {
    return R.noContent(response);
    }
    throw RRException.serverError();
    }
    ...
    */
    @PutMapping("/update")
    public R update(@Valid @RequestBody OrganizationRemind organizationRemind, HttpServletResponse response) {
    if (organizationRemind.getId() == null) {
    throw RRException.badRequest(Constant.ERROR_MSG_ID_NEED);
    }
    if (organizationRemindService.updateById(organizationRemind)) {
    return R.noContent(response);
    }
    throw RRException.serverError();
    }

//    @GetMapping("/selectRemindByName/{name}")
//    public R selectRemindByName(@PathVariable String name){
//        if(organizationRemindService.selectRemindByName(name)){
