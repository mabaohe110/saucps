function select(pageNum){
    ...
    $.ajax({
        url: "/user/userVMListBygBranchId",
        type: "get",
...
function update_add(user) {
    ...
    $.ajax({
        type: "get",
        data: {},
        url: "/party-branch/gBranchList/"+gId,
        dataType:"JSON",
        async: false,
        success: function(data) {
            ...
            for (var i = 0; i < partyBranches.length;i++){
                if(bId == partyBranches[i].id){
                    str+="<option id='"+partyBranches[i].id+"' value='"+partyBranches[i].id+"' selected>"+partyBranches[i].name+"</option>";
                }else{
    ...
    if(confirm(msg) == true){
        $.ajax({
            ...
            success: function(response,status,xmlHttpRequest) {
                ...
                location.href="general_branch_head.html";
            }
        });
    }
}
