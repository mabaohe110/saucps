...
@RequestMapping("/study-link")
public class StudyLinkController {

@Autowired
private IStudyLinkService studyLinkService;
...
Page<StudyLink> page = new Page<>(pageNum, pageSize);
    ...
    */
    ...
    public R info(@PathVariable("id") Integer id) {
        StudyLink studyLink = studyLinkService.getById(id);
    if (studyLink == null) {
    throw RRException.notFound(Constant.ERROR_MSG_ID_NOT_EXIST);
    }
    return R.ok().put("studyLink", studyLink);
    }
    ...
    */
    @PostMapping("/save")
    public R save(@Valid @RequestBody StudyLink studyLink, HttpServletResponse response) {
    if (studyLink.getId() != null) {
    throw RRException.badRequest(Constant.ERROR_MSG_ID_NOT_NEED);
    }
    if (studyLinkService.save(studyLink)) {
    return R.created(response).put("studyLink", studyLink);
    }
    ...
    */
    ...
    if (studyLinkService.removeById(id)) {
    ...
    */
    ...
    if (studyLinkService.removeByIds(Arrays.asList(ids))) {
    return R.noContent(response);
    }
    throw RRException.serverError();
    }
    ...
    */
    @PutMapping("/update")
    public R update(@Valid @RequestBody StudyLink studyLink, HttpServletResponse response) {
    if (studyLink.getId() == null) {
    throw RRException.badRequest(Constant.ERROR_MSG_ID_NEED);
    }
    if (studyLinkService.updateById(studyLink)) {
    return R.noContent(response);
    }
    throw RRException.serverError();
    }
    ...
    public R selectByThemeId(@PathVariable("themeId") int themeId){
        List<StudyLink> studyLinks = studyLinkService.selectByThemeId(themeId);
        if (studyLinks == null) {
            throw RRException.notFound(Constant.ERROR_MSG_ID_NOT_EXIST);
        }
        return R.ok().put("studyLinks", studyLinks);
    }
